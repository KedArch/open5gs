...
OpenAPI_sm_context_retrieve_data_t *OpenAPI_sm_context_retrieve_data_parseFromJSON(cJSON *sm_context_retrieve_dataJSON)
{
    ...
    if (not_to_transfer_ebi_list) {
        ...
        double *not_to_transfer_ebi_list_local_value = (double *)ogs_calloc(1, sizeof(double));
        if(!not_to_transfer_ebi_list_local_value) {
            ogs_error("OpenAPI_sm_context_retrieve_data_parseFromJSON() failed [not_to_transfer_ebi_list]");
            goto end;
        }
        *not_to_transfer_ebi_list_local_value = not_to_transfer_ebi_list_local->valuedouble;
        OpenAPI_list_add(not_to_transfer_ebi_listList, not_to_transfer_ebi_list_local_value);
        }
    }
