...
OpenAPI_hsmf_update_data_t *OpenAPI_hsmf_update_data_parseFromJSON(cJSON *hsmf_update_dataJSON)
{
    ...
    if (eps_bearer_id) {
        ...
        double *eps_bearer_id_local_value = (double *)ogs_calloc(1, sizeof(double));
        if(!eps_bearer_id_local_value) {
            ogs_error("OpenAPI_hsmf_update_data_parseFromJSON() failed [eps_bearer_id]");
            goto end;
        }
        *eps_bearer_id_local_value = eps_bearer_id_local->valuedouble;
        OpenAPI_list_add(eps_bearer_idList, eps_bearer_id_local_value);
        }
    }
    ...
    if (revoke_ebi_list) {
        ...
        double *revoke_ebi_list_local_value = (double *)ogs_calloc(1, sizeof(double));
        if(!revoke_ebi_list_local_value) {
            ogs_error("OpenAPI_hsmf_update_data_parseFromJSON() failed [revoke_ebi_list]");
            goto end;
        }
        *revoke_ebi_list_local_value = revoke_ebi_list_local->valuedouble;
        OpenAPI_list_add(revoke_ebi_listList, revoke_ebi_list_local_value);
        }
    }
