...
OpenAPI_sm_context_updated_data_t *OpenAPI_sm_context_updated_data_parseFromJSON(cJSON *sm_context_updated_dataJSON)
{
    ...
    if (release_ebi_list) {
        ...
        double *release_ebi_list_local_value = (double *)ogs_calloc(1, sizeof(double));
        if(!release_ebi_list_local_value) {
            ogs_error("OpenAPI_sm_context_updated_data_parseFromJSON() failed [release_ebi_list]");
            goto end;
        }
        *release_ebi_list_local_value = release_ebi_list_local->valuedouble;
        OpenAPI_list_add(release_ebi_listList, release_ebi_list_local_value);
        }
    }
    ...
    if (!cJSON_IsString(forwarding_f_teid)) {
    ...
    sm_context_updated_data_local_var = OpenAPI_sm_context_updated_data_create (
        ...
        forwarding_f_teid ? ogs_strdup(forwarding_f_teid->valuestring) : NULL,
