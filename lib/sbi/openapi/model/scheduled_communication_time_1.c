...
OpenAPI_scheduled_communication_time_1_t *OpenAPI_scheduled_communication_time_1_parseFromJSON(cJSON *scheduled_communication_time_1JSON)
{
    ...
    days_of_week = cJSON_GetObjectItemCaseSensitive(scheduled_communication_time_1JSON, "daysOfWeek");
    if (days_of_week) {
        ...
        double *days_of_week_local_value = (double *)ogs_calloc(1, sizeof(double));
        if(!days_of_week_local_value) {
            ogs_error("OpenAPI_scheduled_communication_time_1_parseFromJSON() failed [days_of_week]");
            goto end;
        }
        *days_of_week_local_value = days_of_week_local->valuedouble;
        OpenAPI_list_add(days_of_weekList, days_of_week_local_value);
        }
    }

    time_of_day_start = cJSON_GetObjectItemCaseSensitive(scheduled_communication_time_1JSON, "timeOfDayStart");
    ...
    time_of_day_end = cJSON_GetObjectItemCaseSensitive(scheduled_communication_time_1JSON, "timeOfDayEnd");
    if (time_of_day_end) {
